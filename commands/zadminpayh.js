...
module.exports.run = async (bot, message, args) => {
    ...
    Data.findOne({
        ...
        if(!userData) {
            ...
            userData.heartMoney += parseInt(args[1]);
            userData.save().catch(err => console.log(err));
        }

        return message.channel.send(`❤️ ${message.author.username} admin paid ${args[1]} hearts to ${bot.users.cache.get(user.id).username}.`);
    })
